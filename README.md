...
sqlite is a customized DApp written in Python, which originally resembles the one provided by the sample [Echo Python DApp](https://github.com/cartesi/rollups-examples/tree/main/echo-python).
...
```shell
DAPP_NAME=sqlite docker compose -f ./deploy-testnet.yml up
```

This will create a file at `./deployments/<network>/sqlite.address` with the deployed contract's address.
Once the command finishes, it is advisable to stop the docker compose and remove the volumes created when executing it.

```shell
DAPP_NAME=sqlite docker compose -f ./deploy-testnet.yml down -v
```
...
```shell
yarn start send --input "Hello there" --addressFile path/to/sqlite/deployments/<network>/sqlite.address
```
